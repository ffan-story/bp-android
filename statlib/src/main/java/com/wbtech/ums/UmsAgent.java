...
public class UmsAgent {
    ...
    public static void saveInfoToFile(String type, JSONObject info, Context context) {
        ...
        try {
            if (handler != null) {
                handler.post(new SaveInfo(context, info));
            } else {
     ...
     * Save the event time
     */
    public static void saveEvent(UmsAgent umsAgent, Context context, String appkey, String event_id,
                                 String label, int acc) {
        umsAgentEntity = umsAgent;
        // umsAgentEntity.saveEvent(context, appkey, event_id, label, acc);
        umsAgentEntity.saveEventForHttps(context, event_id);
    }
    ...
    private void saveEventForHttps(Context context, String event_id) {

        String jsonStr = getEventModleForHttps(context, event_id);
        ...
        if (1 == CommonUtil.getReportPolicyMode(context)
            ...
            try {
                JSONObject localJSONObject = getClientDataJSONObjectForHttps(context,
                        event_id);
                String eventUrl = WClickAgentImpl.URL;
                Log.i("", "###eventUrl");
                MyMessage info = NetworkUitlity.postByHttps(eventUrl,
                        localJSONObject);
                if (!info.isFlag()) {
                    saveInfoToFile("content", jsonObject, context);
                    CommonUtil.printLog("error", info.getMsg());
                }
                CommonUtil.printLog("UmsAgent", "errorInfo" + info.getMsg());
            } catch (Exception e) {
        ...
        if (0 == CommonUtil.getReportPolicyMode(context)) {
            CommonUtil.printLog("testnetorbendi", "111111");
            saveInfoToFile("content", jsonObject, context);
        }
        //加上CommonUtil.getReportPolicyMode == 2则是实时上传登录与启动日志的逻辑
        if (2 == CommonUtil.getReportPolicyMode(context)
                && CommonUtil.isNetworkAvailable(context)) {

            CommonUtil.printLog("testnetorbendi", "22222");
            NetworkUitlity.post(WClickAgentImpl.URL + UmsConstants.APPSTATLOGURL, getEventFor_APPLOGLOGINANDLAUNCH(context, event_id), UmsConstants.POST_TYPE_PROMPTLY);

        }
    }

    //获取日志上传的content部分
    private String getEventFor_APPLOGLOGINANDLAUNCH(Context context, String event_id) {
        //InstallAppInfo提供一个没有事件的手机信息数据封装对象
        AppstatlogModel appstatlogModel = InstallAppInfo.getDefultAppInfo();
        GetDeviceInfoUtil deviceInfoUtil = new GetDeviceInfoUtil();
        appstatlogModel.setEvent_log(deviceInfoUtil.getOperations(event_id));
        Gson gson = new Gson();
        String result = gson.toJson(appstatlogModel);
        deviceInfoUtil = null;
        return result;
    }

    private JSONObject getEventJOSNobjForHt(Context context, String event_id) {

        JSONObject eventData = new JSONObject();
        try {
            JSONArray httpsArray = new JSONArray();
            JSONObject httpsObject = new JSONObject();

            httpsObject.put("user_id", WStat.getUserId());//
            httpsObject.put("event_time", CommonUtil.getTime());
            httpsObject.put("event_id", event_id);
            httpsArray.put(httpsObject);
            eventData.put("event_log", httpsArray);

        } catch (JSONException localJSONException) {
            CommonUtil
                    .printLog("UmsAgent", "json error in emitCustomLogReport");
            localJSONException.printStackTrace();
        }
        return eventData;
    }

    //获取一个字符串，这个字符串是一个AppstatlogModel数据源转成的字符串
    private String getEventModleForHttps(Context context, String event_id) {

        GetDeviceInfoUtil getDeviceInfoUtil = new GetDeviceInfoUtil();
        ArrayList<Operation> operations = getDeviceInfoUtil.getOperations(event_id);
        AppstatlogModel appstatlogModel = new AppstatlogModel();
        appstatlogModel.setEvent_log(operations);
        Gson gson = new Gson();
        return gson.toJson(appstatlogModel);
    }


    //传过来一个事件id生辰一个Operation list
    private JSONObject getEventJOSNobjForHttps(Context context, String event_id) {
    ...
    private static void isCreateNewSessionID(Context context) {
        ...
        SharedPreferences preferences =
                context.getSharedPreferences("UMS_session_ID_savetime", Context.MODE_PRIVATE);
    ...
    private static void isupdate(Context context) {
        ...
        if (CommonUtil.isNetworkAvailable(context) && CommonUtil.isNetworkTypeWifi(context)) {
            MyMessage message =
                    NetworkUitlity
                            .post(UmsConstants.preUrl + UmsConstants.updataUrl, updateObject.toString());
            if (message.isFlag()) {
                try {
                    ...
                    if (Integer.parseInt(flag) > 0) {
                        ...
                        UpdateManager manager =
                                new UpdateManager(context, version, forceupdate, fileurl, description);
    ...
    public static void setDefaultReportPolicy(Context context, int reportModel) {
        ...
        if ((reportModel == 0) || (reportModel == 1) || reportModel == 2) {
    ...
    private static void saveSessionTime(Context context) {
        // TODO Auto-generated method stub
        SharedPreferences preferences2sessiontime =
                context.getSharedPreferences("UMS_session_ID_savetime", Context.MODE_PRIVATE);
    ...
    private static void uploadAllLog(Context context) {
        ...
        if (file1.exists()) {
            ...
            try {
                in =
                        new FileInputStream(Environment.getExternalStorageDirectory().getAbsolutePath()
                                + "/mobclick_agent_cached_" + context.getPackageName());
                ...
                if (CommonUtil.isNetworkAvailable(context)) {
                    MyMessage message =
                            NetworkUitlity.post(UmsConstants.preUrl + UmsConstants.uploadUrl, sb + "");
                    if (message.isFlag()) {
                        File file =
                                new File(Environment.getExternalStorageDirectory().getAbsolutePath()
                                        + "/mobclick_agent_cached_" + context.getPackageName());
    ...
    private static void postClientDatas(Context context) {
        if (isFirst) {
            ...
            if (1 == CommonUtil.getReportPolicyMode(context) & CommonUtil.isNetworkAvailable(context)) {
                MyMessage message =
                        NetworkUitlity.post(UmsConstants.preUrl + UmsConstants.clientDataUrl,
                                clientData.toString());
                if (!message.isFlag()) {
    ...
    private static JSONObject getClientDataJSONObjectForHttps(Context context, String event_id) {
        ...
        try {
            JSONArray httpsArray = new JSONArray();
            JSONObject httpsObject2 = new JSONObject();
            JSONObject httpsObject3 = new JSONObject();
            JSONObject httpsObject4 = new JSONObject();

            clientData.put("device_id", tm.getDeviceId() == null ? "" : tm.getDeviceId());
            clientData.put("screen_resolution_desc", displaysMetrics.widthPixels + " * "
                    + displaysMetrics.heightPixels);
            clientData.put("horizontal_flag", CommonUtil.getHorizontalFlag(context));//
            clientData.put("os_version", "Android " + CommonUtil.getOsVersion(context));
            clientData.put("mobile_operators_desc", CommonUtil.getMobileOperators(context));// 移动运营商
            clientData.put("network_desc",
                    CommonUtil.isNetworkTypeWifi(context) ? "Wifi" : CommonUtil.getNetworkType(context));
            clientData.put("sdk_version", CommonUtil.getSDKVersion());
            clientData.put("device_desc", CommonUtil.getDeviceDesc(context));
            clientData.put("app_source", WStat.getApp_source());// 源程序
            clientData.put("app_version", CommonUtil.getAppVersion(context));
            // Log.i("", "############ app_version");
            httpsObject2.put("user_id", WStat.getUserId());//
            httpsObject2.put("event_time", CommonUtil.getTime());
            httpsObject2.put("event_id", event_id);
            httpsArray.put(httpsObject2);
            clientData.put("event_log", httpsArray);
            // Log.i("", "############ ClientData = "+clientData.toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return clientData;
    ...
    public static JSONObject getBasicClientDataForHttps(Context context) {
        ...
        try {
            ...
            clientData.put("horizontal_flag", CommonUtil.getHorizontalFlag(context));//
            clientData.put("os_version", "Android " + CommonUtil.getOsVersion(context));
            clientData.put("mobile_operators_desc", CommonUtil.getMobileOperators(context));
            clientData.put("network_desc",
                    CommonUtil.isNetworkTypeWifi(context) ? "Wifi" : CommonUtil.getNetworkType(context));
            clientData.put("sdk_version", CommonUtil.getSDKVersion());
            clientData.put("device_desc", CommonUtil.getDeviceDesc(context));
    ...
    private static JSONObject getClientDataJSONObject(Context context) {
        ...
        try {
            ...
            clientData.put("deviceid",
                    tm.getDeviceId() == null ? "" : tm.getDeviceId());//
            ...
            clientData.put("wifimac", wifiManager.getConnectionInfo()
                    .getMacAddress());
            ...
            LatitudeAndLongitude coordinates = CommonUtil
    ...
    private static void postonResume(Context context) {
        if (!CommonUtil.isNetworkAvailable(context)) {
            ...
            //此处给强制是定量的，定时的在闹钟打开的服务中调取，直接调用另一个定时子类
            QuantitativeThread thread = new QuantitativeThread(context.getPackageName());
            thread.start();
            GetInfoFromFileThreadPool.getInstance().putThread(thread);
        }
     ...
     * if(1==CommonUtil.getReportPolicyMode(context)&&CommonUtil.isNetworkAvailable(context)){
     * //CommonUtil.printLog("content", info.toString());
     * //MyMessage message = NetworkUitlity.post(UmsConstants.preUrl+UmsConstants.activityUrl,
     * info.toString());
     * //add by tianjun
     * MyMessage message = NetworkUitlity.postByHttps(WClickAgentImpl.URL, info);
     * if(!message.isFlag()){
     * saveInfoToFile("content", info, context);
     * CommonUtil.printLog("error", message.getMsg());
     * }
     * }else{
     * saveInfoToFile("content", info, context);
    ...
    private static void postErrorInfo(Context context, String error) {
        ...
        if (1 == CommonUtil.getReportPolicyMode(context) && CommonUtil.isNetworkAvailable(context)) {
            ...
            MyMessage message =
                    NetworkUitlity.post(UmsConstants.preUrl + UmsConstants.errorUrl, errorInfo.toString());
            if (!message.isFlag()) {
